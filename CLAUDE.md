...
**Current Version**: Read from `VERSION.TXT` (single source of truth). Currently 1.3.1.
...
# Building
cd scripts/build && python build_exe.py             # Build exe (PyInstaller, registry mode)
cd scripts/build && python build_exe.py --portable  # Build portable exe (writes _build_info.py with IS_PORTABLE=True)
...
**Unit Tests** (`tests/`): Split by domain — `test_core.py` (INI parsing/merging/category extraction; `TestStringEntry` is currently `@pytest.mark.skip` because its constructor calls predate `category` and `status` becoming required positional args — fix is a separate cleanup), `test_missions.py` / `test_mission_engagement.py` / `test_mission_turrets.py` (mission rewards pipeline + mission metadata enrichment), `test_pak_extraction.py` (P4K/DataForge), `test_progress_sink.py` (thread-safe progress coalescing), `test_dataforge_patcher.py` (declarative XML patching), `test_app_updater.py` (GitHub Releases version-check worker), `test_channel_layout.py` (per-channel directory migration), `test_retired_url_sources_migration.py` (1.0 cleanup of the contracts/components/ships/commodities/gear sources retired in 0.7.0 — covers fresh-install defaults, upgrade-time pruning, URL-vs-local guard, and idempotence), `test_applied_file_validator.py` (post-apply `global.ini` vs stock `base.ini` validation), `test_entry_filter.py` (column-filter logic + the `NUM_COLUMNS` getter-tuple drift guard), `test_markdown_renderer.py` (About/Help markdown→HTML conversion), `test_resource_path.py` (PyInstaller `_MEIPASS`-aware resource resolution), `test_status_classification.py` (modified/unmodified/favorite status derivation for the table), `test_user_cfg.py` (`g_language` ensure logic, no-dup guarantee), `test_user_ini_autosave_guard.py` (close-time autosave must not truncate a populated user.ini), `test_frontend_version_stamp.py` (Smart Citizen watermark appended to `Frontend_PU_Version` on apply), `test_json_settings.py` (portable-mode JSON settings backend, including slash-prefixed keys QSettings IniFormat mangles), `test_portable_mode.py` (`AppSettings` routing when `IS_PORTABLE=True` — backend swap + data-dir override), `test_build_info_fallback.py` (default `IS_PORTABLE=False` when `_build_info.py` is absent), `test_locpack_exporter.py` (Export Loc-Pack zip layout + filename default). Worker classes themselves have no automated tests — they need `pytest-qt` (not currently a dev dependency); manual smoke testing is still the only verification path for the QThread workers in `workers.py`. Pytest config lives in `tests/pytest.ini` — `pythonpath = . src` (project root for `from src.X` imports + `src/` for legacy `from utils.X` imports used by older tests), registers markers: `unit`, `integration`, `slow`, `critical`, `regression`.
...
- `build_mode.py` — Single `IS_PORTABLE` flag. Production code only reads it; the build script overwrites it (via a generated `_build_info.py` that is git-ignored) for portable builds. Tests can flip the flag via `monkeypatch.setattr` without rebuilding.
...
| Change Loc-Pack export (toolbar zip action) | `locpack_exporter.py`, `main_window.py` | `write_locpack_zip()`, `MainWindow.export_locpack` |
| Change the in-game `Frontend_PU_Version` watermark | `main_window.py` | search `_FRONTEND_VERSION_KEY` |
| Change DataForge diff/skip behaviour | `dataforge_diff.py`, `pak_extractor.py` | `CATEGORY_SUBTREES` ↔ `DATAFORGE_KEEP_SUBPATHS` (keep in sync), `dirty_categories()` |
| Change portable / registry routing | `build_mode.py`, `json_settings.py`, `settings.py` | `IS_PORTABLE`, `JsonSettings`, `AppSettings._backend` |
