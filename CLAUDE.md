...
| Help/About content | `docs/HELP.md`, `docs/ABOUT.md` — bundled under `docs/` in the frozen build via `SmartCitizen.spec` (and `scripts/build/build_exe.py` for the CLI path); rendered inside the in-app help panel via `get_resource_path("docs/HELP.md")` so dev and frozen resolve symmetrically |
| Legal tab content | `docs/LEGAL.md` — CIG community-content compliance, license summaries, privacy/data-handling disclosure, AI-use statement; bundled via `SmartCitizen.spec` (source `docs/LEGAL.md`, bundle dest `docs`) and rendered through the same markdown→HTML pipeline as About |
