...
- `build_mode.py` — `IS_PORTABLE: bool` flag. Default `False`; `scripts/build/build_exe.py --portable` writes a tiny generated `src/utils/_build_info.py` (git-ignored) with `IS_PORTABLE = True` and `build_mode.py` picks it up via `try: from src.utils._build_info import IS_PORTABLE`. The build script deletes the generated file post-build so the source tree never gets committed in portable-mode state. Tests flip the flag via `monkeypatch.setattr` to exercise both branches; production code only ever reads it.
- `json_settings.py` — `JsonSettings`: file-backed key-value store mirroring the four `QSettings` methods `AppSettings` actually uses (`value` / `setValue` / `remove` / `sync`). Thread-safe (locked) and persists every write immediately so a mid-session crash doesn't lose user-visible settings. Used in portable mode as the `AppSettings._backend` instead of `QSettings`. The reason this isn't `QSettings(IniFormat)`: QSettings's INI writer mangles slash-prefixed keys into nested groups and can't round-trip them — and the app uses keys like `enhancements/categories/foo/enabled`.
- `dataforge_diff.py` — Manifest-based diff cache over the DataForge XML tree. After unforge writes the cache, call `update_manifest(cache_dir)` to snapshot file hashes into `.diff_manifest.json`; before re-running enhancement generators, call `dirty_categories(cache_dir)` which returns `None` (no prior manifest — run everything), an empty set (nothing changed — skip everything), or a subset of `CATEGORY_SUBTREES` keys that need regeneration. `CATEGORY_SUBTREES` maps each category (ships, components, ship_weapons, fps_weapons, missions, commodities, journal) to the DataForge subtree prefixes it reads from — it mirrors `DATAFORGE_KEEP_SUBPATHS` in `pak_extractor.py` and must be kept in sync. Hash sweep oversubscribes worker count (~2× CPU) because the ~28k tiny XMLs on Windows hit Defender / Search Indexer / OneDrive per open and are I/O-bound, not CPU-bound.
- `locpack_exporter.py` — Backs the "Export Loc-Pack" toolbar action. `default_locpack_filename(channel, today=None)` → `SmartCitizen-LocPack-{channel}-{YYYYMMDD}.zip`; `write_locpack_zip(source_global_ini, output_zip_path)` zips the already-applied `global.ini` (read from the game's localization directory) for sharing on Discord / org sites. Recipients drop the unzipped `global.ini` into their own `StarCitizen\<channel>\data\Localization\english\` — Smart Citizen not required to consume. Pure file-I/O so it's testable without Qt.
