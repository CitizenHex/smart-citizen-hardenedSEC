...
def fetch_latest_signed_release() -> dict:
    ...
    manifest = verify_release_manifest(
        _read(manifest_asset["url"], "application/octet-stream"),
        _read(signature_asset["url"], "application/octet-stream"),
    )
    ...
    return {"manifest": manifest, "release_url": release.get("html_url", ""), "zip_url": zip_asset.get("url", "")}
