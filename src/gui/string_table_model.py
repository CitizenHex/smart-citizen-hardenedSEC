...
def _make_sort_key(entries, default_values, sort_keys, col, grouped, favorite_prefix,
    ...
    if col == COL_ORDER:
        ...
        def order_key(idx):
            e = entries[idx]
            is_name = e.category == "Ships" and is_ship_name_key(e.key)
            order = get_order(e.custom_value, favorite_prefix) if is_name else ""
...
class StringTableModel(QAbstractTableModel):
    ...
    def flags(self, index: QModelIndex) -> Qt.ItemFlag:
        ...
        if col == COL_STAR:
            ...
            if entry is not None and (entry.category != "Ships" or not is_ship_name_key(entry.key)):
                return Qt.ItemFlag.ItemIsEnabled  # not selectable
        if col == COL_ORDER:
            entry = self.entry_for_row(index.row())
            if entry is not None and entry.category == "Ships" and is_ship_name_key(entry.key):
                return base | Qt.ItemFlag.ItemIsEditable
    ...
    def data(self, index: QModelIndex, role=Qt.ItemDataRole.DisplayRole):
        ...
        if role == Qt.ItemDataRole.ForegroundRole:
            if col == COL_STAR and entry.category == "Ships" and is_ship_name_key(entry.key):
        ...
        if role == Qt.ItemDataRole.BackgroundRole:
            if (
                ...
                and entry.custom_value.startswith(prefix)
            ):
                return self._fav_bg
            return None
