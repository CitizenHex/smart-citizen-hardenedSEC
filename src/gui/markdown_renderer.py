...
def _convert_markdown_inline(text: str) -> str:
    ...
    # Links — do before bold/italic so '_' inside URLs doesn't get chewed.
    ...
    # Italic: *...* (only; '_' is too common in loc-keys/identifiers
    # to italicize safely without a proper tokenizer). Require no '*'
    # on either side of the pair so we don't steal halves of a '**'
    # bold run that happened to not match the bold step.
    text = re.sub(r"(?<!\*)\*(?!\*)([^*\n]+?)\*(?!\*)", r"<em>\1</em>", text)

    # Restore code spans — escape the content so any stray angle
...
def markdown_to_html(
    ...
    html += (
        f"blockquote {{ border-left: 4px solid {link_color}; padding-left: 15px; "
        f"font-style: italic; font-size: 15px; }}"
    )
