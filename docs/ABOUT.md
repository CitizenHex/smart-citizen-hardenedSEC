...
- **Mission Annotations**: `[BP]` / `[BP?]` blueprint reward tags on titles, plus structured *MISSION DETAILS*, *POTENTIAL BLUEPRINTS*, and *ITEM REWARDS* blocks in descriptions. Reputation tier lines show actual rank names (Rookie, Jr. Contractor, etc.) instead of generic numbering. Mission XP names the reputation track it feeds, and Battaglia scan/mine titles carry `[RS ####]` resource-signature tags, optionally joined by a MISSION DETAILS "Resource Signatures" breakdown per targeted ore and an ore-name annotation shown everywhere the game displays that name, including the mission tracker (both off by default, independent toggles)
