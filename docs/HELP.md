...
- **Also scan LIVE/HOTFIX (whichever isn't active)** checks whichever of those two isn't your current channel too, since they share the same account progression — a blueprint earned on LIVE shows up in HOTFIX's logs and vice versa. Off by default (a second channel's full log history is extra work to read). PTU, EPTU, and TECH-PREVIEW are separate test builds with their own progression and are never scanned, regardless of this toggle.
