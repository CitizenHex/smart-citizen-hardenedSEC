...
- **Show RS in ore names** — appends each mineable ore's base Resource Signature to its own display name (e.g. "Aluminium (RS 4285)"), so it shows up everywhere the game renders that name, including the mission tracker. Independent of the Resource Signatures line under Mission Details fields below; off by default.
