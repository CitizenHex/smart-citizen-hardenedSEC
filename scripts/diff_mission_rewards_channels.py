...
Reads from ``Documents\\Smart Citizen\\<channel>\\cache\\mission_rewards_enhancements.ini``;
...
def write_report(
    ...
    *,
    ...
    if full and new_titles_no_bp:
        ...
        out_lines.append(
            f"_({len(new_titles_no_bp)} titles — pass `--full` to list them)_"
            if new_titles_no_bp else "_(none)_"
        )
        out_lines.append("")
